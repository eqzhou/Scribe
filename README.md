...
- **AI 批量生成** — 新建作品时按简版 / 标准 / 详细生成小说蓝图，预览确认后写入世界观、角色、场景、剧情线、剧情节点、伏笔、灵感与章节草案；章节全文被用户接受后自动回写章节摘要，并同步角色、场景、剧情、世界观、灵感与伏笔状态
...
| 全文生成 | 编辑器工具栏（弹窗输入大纲） | SSE 流式 Ghost Text；用户接受后触发章节结构同步 |
| 章节大纲生成 | 大纲面板 | 非流式 JSON |
| 角色对话 | 角色档案页 | SSE 流式 |
| 单条世界观构建 | 世界观条目编辑器 | SSE 流式 |
| 项目蓝图生成 | 新建作品时「生成蓝图预览」按钮 | 非流式 JSON，支持架构深度、分类审阅、移除、重新生成；确认后由服务端事务批量入库 |
...
- 项目蓝图确认：验证架构深度随请求提交、确认前零写入、筛选条目生效、世界观双向关系、文件名边界与导入限流，以及事务中途失败不残留半成品作品。
