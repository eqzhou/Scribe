...
- **AI 批量生成** — 作品创建时一键生成小说蓝图，写入世界观、角色、场景、剧情线、剧情节点、灵感与章节草案；CharacterForm 基于 prompt 生成角色档案；章节全文生成完成后自动同步章节结构副产物
...
| 项目蓝图生成 | 作品创建时「创建并生成架构」按钮 | 非流式 JSON，自动写入角色 / 世界观 / 场景 / 剧情 / 灵感 / 章节草案 |
| 章节结构同步 | 章节全文生成完成后自动触发 | 非流式 JSON，自动写入角色 / 场景 / 剧情节点 / 世界观 / 灵感 |
